...
- **Nghi ngờ:** lọt hết. `/apply` **không có limiter riêng** — chỉ `apiLimiter` 600/phút toàn `/api` (`index.js:28,37`), trong khi `/auth/login` có `authLimiter` 20 lần/15 phút (`:32-36`). `/api/public` lại dùng parser **16MB** (`:24`). → **~9.6GB/phút** đẩy vào Postgres + S3, mỗi đơn hợp lệ tạo tới **2 object trong bucket CCCD**, từ người **hoàn toàn ẩn danh**. Không chống trùng: cùng tên+SĐT gửi 500 lần ra 500 đơn `pending`. `pref`, `note`, `code`, `class_name`, `plate` (`:116-117`) là TEXT **không giới hạn độ dài**.
